...
`;
  ...
  // 1. 各プランのコンパイルを並列実行
  const similarDrafts = llmRes.similarPlans ?? [];
  const [original, recommended, ...similars] = await Promise.all([
    compilePlan(llmRes.originalPlan, 'original-plan', 'original', '原案プラン'),
    compilePlan(llmRes.recommendedPlan, 'recommended-plan', 'recommended', 'おすすめプラン'),
    ...similarDrafts.map((draft, i) =>
      compilePlan(draft, `similar-plan-${i}`, 'similar', '候補プラン'),
    ),
  ]);

  const plans: TravelPlan[] = [original, recommended, ...similars];
